...
moduleToText settings m =
  ...
  where
    otherImports =
        importsFromList
            (lensImports settings <> genericsImports <> argonautCodecsImports settings <> foreignImports settings)
...
foreignImports :: Switches.Settings -> [ImportLine]
foreignImports settings
...
sumTypeToText settings st =
    T.pack (show (sumTypeToTypeDecls st)) <> additionalCode
  where
    additionalCode :: Text
    additionalCode =
