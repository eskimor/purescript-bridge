...
import           Data.List (groupBy, nubBy, sort, sortBy)
...
typeToImportLines t =
    unionImportLines (typesToImportLines $ Set.fromList (_typeParameters t)) $
        importsFromList [ImportLine (_typeModule t) Nothing (Set.singleton (_typeName t))] -- Alias
...
moduleToText m = flip mappend "\n" . renderText . vsep $
    ...
    <> punctuate (line <> dashes <> line) (sumTypeToDocs =<< psTypes m)
    where
...
sumTypeToTypeDecls st@(SumType t cs _) =
  line <> vsep (typeDecl : instances st)
  where
    typeDecl
...
instances st@(SumType t _ is) = go <$> (sort is)
  where
    ...
    -- Foreign Generic uses `Generic` below
...
sumTypeToOptics st =
    line <> vsep (constructorOptics st <> recordOptics st)

constructorOptics :: SumType 'PureScript -> [Doc]
constructorOptics (SumType t cs _) =
    line : (constructorToOptic (length cs > 1) t <$> cs)
